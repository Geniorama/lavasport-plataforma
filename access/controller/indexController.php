...
class indexController
{
	...
	public function __construct()
	{
		...
		$this->tradu = new GoogleTranslate();
	}
	...
	public function index()
	{
		...
		if (ifaut()) {
			$llega["fecha1"] = $_POST["fecha1"];
			$llega["fecha2"] = $_POST["fecha2"];

			$pagos = $this->obj->verpagos($llega["fecha1"], $llega["fecha2"]);
			$facturas = $this->obj->verfacturas($llega["fecha1"], $llega["fecha2"]);
	...
	{
		if ($this->valor == 1) {
			$_SESSION['JC_Idioma'] = 'esco';
		} else {
			$_SESSION['JC_Idioma'] = 'enus';
		}
	...
	public function validadrupal741($clave = 'Jrd52743.', $resultado = '$S$DpLSiVUq7w.EkhAxhW2TUNNhSW/Bc/cDbBt3ozxu0JZ9NYDDm.Jc')
	{
		...
		$cadenita = substr($resultado, 4, 8);
		$count = 32768;
		$hash = hash('sha512', $cadenita . $clave, TRUE);
		do {
			$hash = hash('sha512', $hash . $clave, TRUE);
		} while (--$count);
