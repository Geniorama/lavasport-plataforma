 ...
 <section class="content">
      ...
      <div class="box box-default">
        ...
        <form action="<?php echo PATO; ?>productocupos/facturacion2filtro/" method="post" name="ciudades-filtrar" id="ciudades-filtrar" class="form-inline navbar-text">
...
<?php if(isset($_POST["fecha1"])){ 
  ...
  ?>
    ...
    $nprecio=$sale->fields["valor"];
    if(is_numeric($sale->fields["porcentmod"]) && $sale->fields["porcentmod"]>0){
     $porcentaje=$sale->fields["porcentmod"]/100;

     $nprecio=$sale->fields["valor"]* $porcentaje;
     $nprecio=$sale->fields["valor"]+$nprecio;
    }
    if(is_numeric($sale->fields["porcentmod"]) && $sale->fields["porcentmod"]<0){
     $porcentaje=abs($sale->fields["porcentmod"])/100;
       ...
       <td><?php echo $sale->fields["fecha"]; ?></td>    
            <td><?php echo $sale->fields["documento"]; ?></td>    
              <td><?php echo $sale->fields["nombre"]; ?></td>    
                <td class="text-right"><?php echo number_format($nprecio-$valoriva,0,',','.'); ?></td>    
                  <td class="text-right"><?php echo number_format($valoriva,0,',','.'); ?></td>    
                    <td class="text-right">19%</td>    
                    <td class="text-right"><?php echo number_format($nprecio,0,',','.'); ?></td>         
...
window.open("<?php echo PATO; ?>productocupos/impresionfactura2/"+id, "Impresion", "width=10, height=10")
