...
<h4><?php echo __("Nuevo Productocupo"); ?></h4>

<form id="productocupos-agregando" name="productocupos-agregando" class="form-horizontal" method="post" action="<?php echo PATO; ?>productocupos/agregando/" enctype="multipart/form-data">
...
<div class="form-group row">
<label for="cantidad" class="col-2"><?php echo __("Cantidad"); ?></label>
<div class="col-7"><input id="cantidad" name="cantidad" type="text" class="form-control" data-placement="right" data-original-title="<?php echo __("Dato requerido"); ?>" data-content="<?php echo __("Por favor escriba la Cantidad"); ?>" required /></div>
</div>
...
<div class="col-7"><select name="cupo_id" id="cupo_id" class="custom-select" data-placement="right" data-original-title="<?php echo __("Dato requerido"); ?>" data-content="<?php echo __("Por favor seleccione el Cupo"); ?>" required>
		<option value="0" selected="selected"><?php echo __("Seleccione Cupo"); ?></option><?php
if(!$cupos->EOF){
	while(!$cupos->EOF){ ?>
    	<option value="<?php echo $cupos->fields["id"]; ?>"><?php echo $cupos->fields["nombre"]; ?></option><?php
		$cupos->MoveNext();
	}
	$cupos->Move(0);
} ?></select></div>
...
$(function(){
	...
	$("#productocupos-agregando").submit(function(){
