...
<div>
<h4><?php echo __("Organizaciónes"); ?> <span class="badge badge-primary"><?php if(isset($sale->_numOfRows)){echo $sale->_numOfRows;}else{echo "0";} ?></span></h4>

<nav class="navbar navbar-default" role="navigation">
<div class="navbar-header">

<form action="<?php echo PATO; ?>organizaciones/filtrar/" method="post" name="organizaciones-filtrar" id="organizaciones-filtrar" class="form-inline navbar-text">
...
if($msg==1)echo __("No hay Organizaciónes");
if($msg==2)echo __("Para ver resultados por favor filtre primero");
?></div></div><?php }else{ ?>
<div class="table-responsive"><table border="0" id="tb_sale_organizaciones" class="table table-bordered table-striped table-hover table-condensed">
<thead>
	<tr class="hidden">
...
<a href="<?php echo PATO; ?>organizaciones/ver/<?php echo $sale->fields["id"]; ?>/" data-toggle="tooltip" title="<?php echo __("Ver"); ?>" class="btn btn-default"><i class="fas fa-eye"></i></a>

<a href="<?php echo PATO; ?>organizaciones/editar/<?php echo $sale->fields["id"]; ?>/" data-toggle="tooltip" title="<?php echo __("Editar"); ?>" class="btn btn-default"><i class="fas fa-edit"></i></a>

<a href="JavaScript:;" onClick="eliminarorganizaciones(<?php echo $sale->fields["id"]; ?>)" data-toggle="tooltip" title="<?php echo __("Eliminar"); ?>" class="btn btn-default"><i class="fas fa-trash"></i></a>
...
$(function(){
	...
	$("#tb_sale_organizaciones").tablesorter({headers: {1: {sorter: false}}, widgets: ["zebra"]});
	$("#tb_sale_organizaciones").tablesorterPager({container:$(".pager2"),ajaxUrl:null,output:"{startRow} <?php echo __("a"); ?> {endRow} <?php echo __("de un total de"); ?> {totalRows}",updateArrows:true,page:0,size:<?php echo RPP; ?>,fixedHeight:false,removeRows:true,cssNext:".next",cssPrev:".prev",cssFirst:".first",cssLast:".last",cssPageDisplay:".pagedisplay",cssPageSize:".pagesize",cssDisabled:"disabled"});
});
function eliminarorganizaciones(a){
	confirma1('<?php echo iinterro().__("Esta seguro de eliminar Organizacion?"); ?>','<?php echo __("Si"); ?>','eliminandoorganizaciones',a)}
function eliminandoorganizaciones(a){window.location.href = "<?php echo PATO; ?>organizaciones/eliminar/"+a+"/";}
</script><?php } ?>

<form class="form-inline">

<div class="form-group"><button type="button" class="btn btn-primary form-control" onClick="location.href='<?php echo PATO; ?>organizaciones/agregar/';"><i class="fas fa-plus"></i>&nbsp;&nbsp;<?php echo __("Nuevo"); ?></button></div>
