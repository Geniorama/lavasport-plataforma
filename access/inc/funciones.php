...
{ // Limpia Strings
	$malo = array("\\", ";", "\"", "\'", "'");
	$var = str_replace($malo, "", strip_tags(trim($var)));
	return $var;
}
function limpia2($arr)
{ // Limpia Arrays
	$ya = 0;
	$arr2 = array();
	while (!$ya) {
		$arr2[key($arr)] = limpia(current($arr));
		next($arr);
		if (!key($arr)) $ya = 1;
	}
	return $arr2;
...
function limpia3($arr)
{
	...
	$arr2 = array();
	while (!$ya) {
		$arr2[key($arr)] = limpia(normaliza(urldecode(current($arr))));
		next($arr);
		if (!key($arr)) $ya = 1;
	}
	return $arr2;
...
function iok()
{
	return '<img src="' . PATU . 'img/ok2.png" border="0" class="ijc" /> ';
}
function ierror()
{
	return '<img src="' . PATU . 'img/error2.png" border="0" class="ijc" /> ';
}
function iadmira()
{
	return '<img src="' . PATU . 'img/admiracion2.png" border="0" class="ijc" /> ';
}
function iinterro()
{
	return '<img src="' . PATU . 'img/interrogacion2.png" border="0" class="ijc" /> ';
}
function veraut($a = 1)
{
...
function ifaut()
{
	if (isset($_SESSION['JC_UserID']) && $_SESSION['JC_UserID'] > 0 && $_SESSION['JC_Grupo'] == PERFIL) {
		return true;
	} else {
		return false;
	}
